...
void GPIO_toggle(portX_t portName, pinX_t pinNum)
{
	...
	{
		case PORT_0:
			if(PIN_IS_LOW == GET_BIT(IOPIN0,pinNum))
			{
				SET_BIT(IOSET0, pinNum);
			}
			
			else if (PIN_IS_HIGH == GET_BIT(IOPIN0,pinNum))
			{
				SET_BIT(IOCLR0, pinNum);
			}
			else
		...
		case PORT_1:
			if(PIN_IS_LOW == GET_BIT(IOPIN1,pinNum))
			{
				SET_BIT(IOSET1, pinNum);
			}
			
			else if (PIN_IS_HIGH == GET_BIT(IOPIN1,pinNum))
			{
				SET_BIT(IOCLR1, pinNum);
			}
			else
